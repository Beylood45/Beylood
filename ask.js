...
(function () {
  ...
  function answer(userText) {
    ...
    var res = window.BeyloodKnowledge.search(userText, lang);

    if (res.type === 'empty') {
      return { reply: 'Fadlan qor su\'aal cad.', lang: lang };
    }
    if (res.type === 'greeting' || res.type === 'thanks' || res.type === 'no_match') {
      return { reply: res.response, lang: lang };
    }
    // type === 'match'
    return { reply: window.BeyloodKnowledge.format(res.results, lang), lang: lang };
  }
  ...
  formEl.addEventListener('submit', async function (e) {
    ...
    // Tiny artificial delay so it doesn't feel jarring
